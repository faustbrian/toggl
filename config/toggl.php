...
return [
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Pennant Compatibility Mode
    |--------------------------------------------------------------------------
    |
    | Enable compatibility mode to support gradual migration from Laravel Pennant.
    | When enabled, feature checks will first query Pennant's features table before
    | falling back to Toggl's table. All writes go exclusively to Toggl.
    |
    | This is useful when migrating millions of feature flags over weeks, allowing
    | your application to read from both systems during the transition period.
    |
    */

    'pennant_compatibility' => [
        /*
        |--------------------------------------------------------------------------
        ...
        */
        ...
        'enabled' => env('TOGGL_PENNANT_COMPAT_ENABLED', false),
        /*
        |--------------------------------------------------------------------------
        | Pennant Features Table
        |--------------------------------------------------------------------------
        |
        | The table name where Laravel Pennant stores feature flags. This is
        | typically 'features' but can be customized if you use a different name.
        |
        */
        ...
        |--------------------------------------------------------------------------
        |
        | The database connection where Pennant's features table exists. If null,
        | uses the default database connection. Useful when Pennant features are
        | stored in a different database than Toggl.
        |
        */
    ...
    */
    ...
    |
    | Supported: "string", "uuid", "ulid", "numeric"
    |
    */

    'morph_type' => env('TOGGL_MORPH_TYPE', 'string'),
    /*
    |--------------------------------------------------------------------------
    | Eloquent Models
    |--------------------------------------------------------------------------
        ...
        */
        ...
        'pennant' => [
            ...
            */
            ...
            |----------------------------------------------------------------------
            |
            | When enabled, this option allows the migrator to track which rows
            | have been successfully migrated by updating a timestamp column in
            | the source table. This enables incremental migrations where only
            | unmigrated rows (where the column IS NULL) are processed in
            | subsequent runs. Useful for large datasets migrated over time.
            |
            */

            'migration_tracking' => [
                'enabled' => env('TOGGL_PENNANT_MIGRATION_TRACKING_ENABLED', false),
                'column' => env('TOGGL_PENNANT_MIGRATION_TRACKING_COLUMN', 'migrated_at'),
            ],
        ],
        /*
        |--------------------------------------------------------------------------
        ...
        */
        ...
        'ylsideas' => [
            ...
            */
            ...
            'migration_tracking' => [
                'enabled' => env('TOGGL_YLSIDEAS_MIGRATION_TRACKING_ENABLED', false),
                'column' => env('TOGGL_YLSIDEAS_MIGRATION_TRACKING_COLUMN', 'migrated_at'),
            ],
        ],
    ],
];
