...
test\:docker\:sqlite\:numeric: build
	docker-compose run --rm -e DB_CONNECTION=sqlite -e TOGGL_PRIMARY_KEY_TYPE=id -e TOGGL_MORPH_TYPE=numeric php84 composer test:numeric

test\:docker\:sqlite\:ulid: build
	docker-compose run --rm -e DB_CONNECTION=sqlite -e TOGGL_PRIMARY_KEY_TYPE=ulid -e TOGGL_MORPH_TYPE=ulid php84 composer test:ulid

test\:docker\:sqlite\:uuid: build
	docker-compose run --rm -e DB_CONNECTION=sqlite -e TOGGL_PRIMARY_KEY_TYPE=uuid -e TOGGL_MORPH_TYPE=uuid php84 composer test:uuid
...
test\:docker\:mysql\:numeric: build
	docker-compose up -d mysql
	docker-compose run --rm -e DB_CONNECTION=mysql -e DB_HOST=mysql -e DB_PORT=3306 -e DB_DATABASE=toggl_test -e DB_USERNAME=root -e DB_PASSWORD=password -e TOGGL_PRIMARY_KEY_TYPE=id -e TOGGL_MORPH_TYPE=numeric php84 composer test:numeric

test\:docker\:mysql\:ulid: build
	docker-compose up -d mysql
	docker-compose run --rm -e DB_CONNECTION=mysql -e DB_HOST=mysql -e DB_PORT=3306 -e DB_DATABASE=toggl_test -e DB_USERNAME=root -e DB_PASSWORD=password -e TOGGL_PRIMARY_KEY_TYPE=ulid -e TOGGL_MORPH_TYPE=ulid php84 composer test:ulid

test\:docker\:mysql\:uuid: build
	docker-compose up -d mysql
	docker-compose run --rm -e DB_CONNECTION=mysql -e DB_HOST=mysql -e DB_PORT=3306 -e DB_DATABASE=toggl_test -e DB_USERNAME=root -e DB_PASSWORD=password -e TOGGL_PRIMARY_KEY_TYPE=uuid -e TOGGL_MORPH_TYPE=uuid php84 composer test:uuid
...
test\:docker\:postgres\:numeric: build
	docker-compose up -d postgres
	docker-compose run --rm -e DB_CONNECTION=pgsql -e DB_HOST=postgres -e DB_PORT=5432 -e DB_DATABASE=toggl_test -e DB_USERNAME=postgres -e DB_PASSWORD=password -e TOGGL_PRIMARY_KEY_TYPE=id -e TOGGL_MORPH_TYPE=numeric php84 composer test:numeric

test\:docker\:postgres\:ulid: build
	docker-compose up -d postgres
	docker-compose run --rm -e DB_CONNECTION=pgsql -e DB_HOST=postgres -e DB_PORT=5432 -e DB_DATABASE=toggl_test -e DB_USERNAME=postgres -e DB_PASSWORD=password -e TOGGL_PRIMARY_KEY_TYPE=ulid -e TOGGL_MORPH_TYPE=ulid php84 composer test:ulid

test\:docker\:postgres\:uuid: build
	docker-compose up -d postgres
	docker-compose run --rm -e DB_CONNECTION=pgsql -e DB_HOST=postgres -e DB_PORT=5432 -e DB_DATABASE=toggl_test -e DB_USERNAME=postgres -e DB_PASSWORD=password -e TOGGL_PRIMARY_KEY_TYPE=uuid -e TOGGL_MORPH_TYPE=uuid php84 composer test:uuid
