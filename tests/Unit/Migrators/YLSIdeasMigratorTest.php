...
describe('YLSIdeasMigrator', function (): void {
    ...
    describe('Sad Path', function (): void {
        ...
        test('throws exception and records error when database connection fails', function (): void {
            $this->markTestSkipped('Database connection mocking not supported with current migrator constructor');
            ...
            // Act & Assert
            expect(fn () => $migrator->migrate())->toThrow(RuntimeException::class, 'Connection failed');

            $stats = $migrator->getStatistics();
            expect($stats['features'])->toBe(0);
            expect($stats['contexts'])->toBe(0);
            expect($stats['errors'])->toHaveCount(1);
            expect($stats['errors'][0])->toBe('Migration failed: Connection failed');
        });
    });
