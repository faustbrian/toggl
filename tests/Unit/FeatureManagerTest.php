...
describe('FeatureManager', function (): void {
    ...
    describe('Guest Context', function (): void {
        test('default context resolver returns GuestContext when user is not authenticated', function (): void {
            // Arrange
            $manager = resolve(FeatureManager::class);

            // Act - Access store to get the default context resolver
            $store = $manager->store('array');
            $reflection = new ReflectionClass($store);
            $method = $reflection->getMethod('defaultContext');
            ...
            // Assert - The source of the TogglContext should be a GuestContext
            expect($result)->toBeInstanceOf(TogglContext::class);
            expect($result->source)->toBeInstanceOf(GuestContext::class);
        });

        test('GuestContext serializes to consistent identifier', function (): void {
            // Arrange
            $context1 = new GuestContext();
            $context2 = new GuestContext();

            // Act
            $serialized1 = $context1->serialize();
            $serialized2 = $context2->serialize();

            // Assert
            expect($serialized1)->toBe('__toggl_guest__');
            expect($serialized2)->toBe('__toggl_guest__');
            expect($serialized1)->toBe($serialized2);
        });

        test('FeatureManager serializes GuestContext correctly', function (): void {
            // Arrange
            $manager = resolve(FeatureManager::class);
            $guestContext = new GuestContext();

            // Act
            $result = $manager->serializeContext($guestContext);

            // Assert
            expect($result)->toBe('__toggl_guest__');
        });

        test('feature checks work with GuestContext on unauthenticated routes', function (): void {
            // Arrange
            $manager = resolve(FeatureManager::class);
            $manager->define('public-feature', true);

            // Act - Call active() without explicit context (simulates guest route)
            $isActive = $manager->active('public-feature');

            // Assert
            expect($isActive)->toBeTrue();
        });

        test('feature activation works with GuestContext', function (): void {
            // Arrange
            $manager = resolve(FeatureManager::class);
            $guestContext = new GuestContext();

            // Act
            $manager->activate('guest-feature')->for($guestContext);
            $isActive = $manager->for($guestContext)->active('guest-feature');

            // Assert
            expect($isActive)->toBeTrue();
        });

        test('different features can have different states for GuestContext', function (): void {
            // Arrange
            $manager = resolve(FeatureManager::class);
            $guestContext = new GuestContext();

            // Act
            $manager->activate('feature-a')->for($guestContext);
            // feature-b is not activated

            // Assert
            expect($manager->for($guestContext)->active('feature-a'))->toBeTrue();
            expect($manager->for($guestContext)->active('feature-b'))->toBeFalse();
        });
    });
});
