...
describe('ArrayDriver', function (): void {
    describe('Happy Path', function (): void {
        ...
        test('defines a feature with static value', function (): void {
            ...
            expect($driver->get('static-feature', TogglContext::simple('user', 'test'))->isActiveRestrictive())->toBeTrue();
        });
        ...
        test('gets feature value and caches it', function (): void {
            ...
            // Assert
            expect($result->isActiveRestrictive())->toBeTrue();
        });
        ...
        test('gets cached feature value on second call', function (): void {
            ...
            // Assert
            expect($result1->isActiveRestrictive())->toBeTrue();
            expect($result2->isActiveRestrictive())->toBeTrue();
        ...
        test('sets feature value for all contexts', function (): void {
            ...
            // Assert
            expect($driver->get('test-feature', TogglContext::simple('user1', 'test'))->isActiveRestrictive())->toBeTrue();
            expect($driver->get('test-feature', TogglContext::simple('user2', 'test'))->isActiveRestrictive())->toBeTrue();
            expect($driver->get('test-feature', TogglContext::simple('user3', 'test'))->isActiveRestrictive())->toBeTrue();
        });
        ...
        test('gets all features in bulk', function (): void {
            ...
            // Assert
            expect($results['feature1'][0]->isActiveRestrictive())->toBeTrue();
            expect($results['feature1'][1]->isForbidden())->toBeTrue();
            expect($results['feature2'][0]->isActiveRestrictive())->toBeTrue();
            expect($results['feature2'][1]->isActiveRestrictive())->toBeTrue();
        });
        ...
        test('gets all features with nested closure execution for multiple features and contexts', function (): void {
            ...
            expect(array_map(fn (FeatureValue $s): mixed => $s->toValue(), $results['feature1']))->toBe([true, false, false]);
            expect(array_map(fn (FeatureValue $s): mixed => $s->toValue(), $results['feature2']))->toBe([false, true, false]);
            expect(array_map(fn (FeatureValue $s): mixed => $s->toValue(), $results['feature3']))->toBe([true, true]);
        ...
        test('get method resolves and stores unknown feature returning false', function (): void {
            ...
            // Assert - Verify undefined feature returns null (undefined state)
        ...
        test('get method resolves and stores real feature value', function (): void {
            ...
            expect($result->toValue())->toBe('resolved-admin');
            expect($callCount)->toBe(1);
            ...
            expect($result2->toValue())->toBe('resolved-admin');
    ...
    describe('Edge Cases', function (): void {
        test('stores complex values', function (): void {
            ...
            // Assert
            expect($result->toValue())->toBe(['key' => 'value', 'nested' => ['data' => 123]]);
        });
        ...
        test('distinguishes between false and unknown feature', function (): void {
            ...
            // Assert
            expect($definedResult->isForbidden())->toBeTrue();
            expect($unknownResult->isUndefined())->toBeTrue();
            Event::assertDispatched(UnknownFeatureResolved::class);
        ...
        test('handles multiple contexts for same feature in getAll', function (): void {
            ...
            // Assert
            expect($results['test-feature'][0]->toBool())->toBeTrue();
            expect($results['test-feature'][1]->toBool())->toBeFalse();
            expect($results['test-feature'][2]->toBool())->toBeFalse();
        });
        ...
        test('setForAllContexts clears existing cached states', function (): void {
            ...
            expect($result1->toBool())->toBeTrue();
            expect($result2->toBool())->toBeTrue();
        });
        ...
        test('get method returns exact resolver value including null', function (): void {
            ...
            expect($nullResult->toValue())->toBeNull();
            expect($zeroResult->toValue())->toBe(0);
            expect($emptyStringResult->toValue())->toBe('');
        ...
        test('get method with() closure captures feature and context variables', function (): void {
            ...
            expect($result->toValue())->toBe('processed-complex-context-value');
        ...
        test('get method executes with() helper return path for both unknown and known features', function (): void {
            ...
            expect($unknownResult->isUndefined())->toBeTrue(); // Unknown path: line 149 -> 150-152
            expect($knownResult->toValue())->toBe('value-for-ctx2'); // Known path: line 149 -> 154-156
            Event::assertDispatched(UnknownFeatureResolved::class);
        });
    });
});
