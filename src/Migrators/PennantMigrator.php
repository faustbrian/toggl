...
final class PennantMigrator implements Migrator
{
     ...
     * @param bool                   $migrationTracking Whether to track migration status with timestamps
     * @param string                 $trackingColumn    Column name for migration timestamp tracking
    ...
    public function migrate(): void
    {
        ...
        try {
            ...
            $this->streamRecords(function (stdClass $record) use (&$processed, &$successful): void {
                if (!property_exists($record, 'name') || !is_string($record->name)) {
                    return;
                }

                ++$processed;

                if ($this->migrateRecord($record->name, $record)) {
    ...
    /**
     * Stream all Pennant feature records without loading the entire table into memory.
     *
     * Uses chunkById when an ID column is present, falling back to a cursor.
     */
    private function streamRecords(callable $handler): void
    {
        ...
        if (Schema::connection($connection)->hasColumn($this->table, 'id')) {
            $query->orderBy('id')->chunkById($this->chunkSize, function ($records) use ($handler): void {
        ...
        /** @var LazyCollection<int, stdClass> $records */
    ...
    /**
     * Migrate a single Pennant record into Toggl.
     *
     * @param  string   $featureName The feature name to migrate
     * @param  stdClass $record      The Pennant database record
     * @return bool     True when the record was migrated successfully
     */
    private function migrateRecord(string $featureName, stdClass $record): bool
    {
        try {
            ...
            $this->markRecordAsMigrated($pennantId);
            ...
            $contextDescription = property_exists($record, 'scope') && is_string($record->scope)
                ? $record->scope
                : 'unknown';
            $this->statistics['errors'][] = sprintf("Failed to migrate context '%s' for feature '%s': %s", $contextDescription, $featureName, $throwable->getMessage());
    ...
    private function deserializeContext(string $serializedContext): mixed
    {
        ...
        if (str_contains($serializedContext, '|')) {
            ...
            if ((bool) Config::get('toggl.migrators.pennant.include_soft_deleted', false)) {
    ...
    /**
     * Mark a Pennant record as migrated by updating its timestamp.
     */
