...
final class ArrayDriver implements CanListStoredFeatures, Driver, HasFlushableCache
{
     ...
     * @return array<string, array<int, FeatureValue>> Map of feature names to their resolved states
     */
     ...
     *
     * Unknown features (those without resolvers) return FeatureState::Undefined.
     *
     * @param  string       $feature The feature flag name
     * @param  TogglContext $context The context to evaluate the feature for
     * @return FeatureValue The resolved feature state
     */
    public function get(string $feature, TogglContext $context): FeatureValue
    {
        ...
        if (array_key_exists($feature, $this->resolvedFeatureStates) && array_key_exists($contextKey, $this->resolvedFeatureStates[$feature])) {
            $cached = $this->resolvedFeatureStates[$feature][$contextKey];

            return $cached instanceof FeatureValue ? $cached : FeatureValue::from($cached);
        }
        ...
        if ($context->scope instanceof FeatureScope && array_key_exists($feature, $this->scopes)) {
            ...
            if ($context->scope->matches($featureScope)) {
                $value = $this->scopes[$feature]['value'];
        ...
        if ($value === $this->unknownFeatureValue) {
            return FeatureValue::from(null);
        }

        $featureState = FeatureValue::from($value);
        $this->set($feature, $context, $featureState);
